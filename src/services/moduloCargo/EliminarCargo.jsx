...
const EliminarCargo = ({ cargoId, cargo1, onModalClose, onCargoEliminado }) => {
  const handleClose = () => {
    onModalClose();
  };
  ...
  const handleEliminarCargo = async () => {
    try {
      const response = await axios.delete(`https://localhost:7266/api/Cargo/${cargoId}`);
  ...
  return (
    <Dialog open onClose={handleClose}>
      ...
      <DialogContent>
        ...
        ¿Estás seguro de que deseas eliminar el Cargo <strong>"{cargo1}"</strong>?
          </DialogContentText>
      </DialogContent>
      <DialogActions>
        ...
        <Button onClick={handleEliminarCargo} variant="contained" sx={{ color: '#fff', backgroundColor: '#C84337', '&:hover': { backgroundColor: '#F87171' } }}
          >
            ELIMINAR
          </Button>
      </DialogActions>
