...
const EliminarEmpleado = ({ empleadoId, empleadoNombre, onModalClose, onEmpleadoEliminado }) => {
  const handleClose = () => {
    onModalClose();
  };
  ...
  const handleEliminarEmpleado = async () => {
    try {
      const response = await axios.delete(`https://localhost:7266/api/Empleado/${empleadoId}`);
  ...
  return (
    <Dialog open onClose={handleClose}>
      ...
      <DialogContent>
        ...
        ¿Estás seguro de que deseas eliminar al Empleado <strong>"{empleadoNombre}"</strong>?
          </DialogContentText>
      </DialogContent>
      <DialogActions>
        ...
        <Button onClick={handleEliminarEmpleado} variant="contained" sx={{ color: '#fff', backgroundColor: '#C84337', '&:hover': { backgroundColor: '#F87171' } }}
          >
            ELIMINAR
          </Button>
      </DialogActions>
