...
const EliminarRol = ({ rolId, tipo, onModalClose, onRolEliminado }) => {
  const handleClose = () => {
    onModalClose();
  };
  ...
  const handleEliminarRol = async () => {
    try {
      const response = await axios.delete(`https://localhost:7266/api/Rol/${rolId}`);
  ...
  return (
    <Dialog open onClose={handleClose}>
      ...
      <DialogContent>
        ...
        ¿Estás seguro de que deseas eliminar el rol <strong>"{tipo}"</strong>?
          </DialogContentText>
      </DialogContent>
      <DialogActions>
        ...
        <Button onClick={handleEliminarRol} variant="contained" sx={{ color: '#fff', backgroundColor: '#C84337', '&:hover': { backgroundColor: '#F87171' } }}
          >
            ELIMINAR
          </Button>
      </DialogActions>
